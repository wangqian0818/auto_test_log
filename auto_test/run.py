...
def main():
    ...
    try:  # 导入版本配置等文件
        ...
        try:
            ...
            mode_list = info.mode_list  # 获取测试模式，包含ssh、rabbitmq
            ...
            try:
                try:
                    ...
                    for i in case_list:
                        ...
                        log_txt_path = abs_path + '\\Logs\\' + str(version) + '\\' + current_time + '\\' + mode + '\\'
                        os.makedirs(log_txt_path)
                        open(log_txt_path + log + '.txt', 'a+')
                        # os.system('pytest ' + i + ' -o log_file=' + log_txt_path + log + '.txt')

                        os.system('pytest ' + i + ' --html=' + abs_path + '/Logs/' + str(version) + '/' + current_time
                                  + '/' + mode + '/' + log + '.html --self-contained-html -o log_file=' + log_txt_path
                                  + log + '.txt')
                        file.write("\n--------------------- 执行结束 case：%s\n\n\n" % i)
                        file.flush()
